...
final as (
    ...
    select 
        application.*,
        {{ dbt_utils.star(from=ref('int_greenhouse__candidate_users'), except=["candidate_id", "new_candidate_id", "created_at"], relation_alias="candidate") }}
        ,
        ...
        {% if var('greenhouse_using_prospects', true) %}
        , prospect_pool.prospect_pool as prospect_pool,
    ...
    left join job_stage
        on application.current_stage_id = job_stage.stage_id
    left join source
        on application.source_id = source.source _id
    left join activity
